...
var op = document.getElementById("enc");
...
function addbin(a, b) {
    var temp = parseInt(a,16)+parseInt(b,16);
...
function f(text){
    ...
    var h =0;
    for (let i = 0; i < 4; i++) {
        arr2[i] = text.substring(h,h+2);
        h=(h+2)%text.length;
        arr2[i] = parseInt(arr2[i],16);
        arr2[i] = s[i][arr2[i]];
    }
    var ans;
    ans = addbin(arr2[0],arr2[1]);
    ans = xor(ans,arr2[2]);
    ans = addbin(ans,arr2[3]);
    return ans;
}

function round(num, plainText) {
    var left = plainText.substring(0,8);
    var right = plainText.substring(8,16);
    left = xor(left, p[num]);
...
function split(plaintext){
    subkey(key.value);
    var text = plaintext2hex(plaintext);
    ...
    if(text.length<16){
        for (let k = 0; k<=16-text.length; k++) {
            arr[0] = "0"+text; 
            op.innerHTML = encrypt(arr[0]);
        }
    }
    else{
        for (var i =0; j>0; i++,j-=16) {
            arr[i] = text.substring(k,k+16);
            k = (k+16)%text.length;
        }
        if(arr[i-1].length<16){
            for (let k = 0; k<=16-arr[i-1].length; k++) {
                arr[i-1] = "0"+arr[i-1]; 
            }
        }
        var final = encrypt(arr[0]);
...
function merge(ciphertext) {
    ...
    var arr3 = [];
        ...
        var k =0;
        for (var i =0; j>0; i++,j-=16) {
            arr3[i] = ciphertext.substring(k,k+16);
            k = (k+16)%ciphertext.length;
        }
        var final = hex2string(decrypt(arr3[0]));
        for (let j = 1; j < i; j++) {
